...
function AddServer( type, id, ping, name, desc, map, players, maxplayers, botplayers, pass, lastplayed, address, gamemode, workshopid )
{
	...
	if ( data.players >= 16 ) data.recommended -= 40;		data.recommended -= Math.min(data.players, 40);
	if ( data.players >= 32 ) data.recommended -= 20;		data.recommended -= Math.min(data.players, 30);
	if ( data.players >= 64 ) data.recommended -= 10;		data.recommended -= Math.min(data.players, 20);
